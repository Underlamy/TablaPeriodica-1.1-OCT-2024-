...
<div id="formulario">
    ...
    <br>
    <div class="nombre allCaps">ELIMINAR <?= $columna ?></div>
    <br>

    <form method="POST" action="tabla/eliminar.php">
        <?php
        ...
        while ($dato2 = mysqli_fetch_assoc($resultadoQueryHTML)) {
            if (!$dato2["COLUMN_KEY"]) {
                ...
                <div class="button">
                    <input type="text" name="txt<?= $dato2['COLUMN_NAME'] ?>" class="inputBox" value="<?= $data[$name] ?>" readonly>
                </div><br>
        <?php }
        }
        ?>
        ...
        <input type="submit" name="btnRegistrarCalificacions" id="btnFORM" value="ELIMINAR">
        <br><br>
    </form>
</div>